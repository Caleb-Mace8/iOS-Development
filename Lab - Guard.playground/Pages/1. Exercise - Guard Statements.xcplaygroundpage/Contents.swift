...
let myArea = calculateArea(x: 2, y: 3)
if let myArea {
    print(myArea)
}
let aArea = calculateArea(x: -20, y: 3)
if let aArea {
    print(aArea)
}
...
let proper = add(x: 2, y: 3)
let invalid = add(x: 2, y: nil)
if let proper {
    print(proper)
}
if let invalid {
...
if let createUser = createUser() {
    print("Hello \(createUser.firstName) \(createUser.lastName), you are \(createUser.age).")
}
/*:
